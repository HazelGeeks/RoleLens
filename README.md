...
- `PYTHON_SCRAPED_SOURCE_TYPE` (optional: `COMPANY_SITE`/`LINKEDIN`/`INDEED`/`MANUAL`, default: `MANUAL`)
...
On the app list screen, `Sync Crawled Feed` can be used for manual refresh.
...
2. Set at least one source for both `Production` and `Preview` scopes:
   - Python feed: `PYTHON_SCRAPED_FEED_URL`
...
5. Open Jobs page and run `Sync Crawled Feed` again.
...
Minimum local feed setup in `.env.local`:
